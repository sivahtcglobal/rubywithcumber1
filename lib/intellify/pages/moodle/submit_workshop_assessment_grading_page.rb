class CourseWorkshopGradingPage < BasePage
  ...
  #Switch to the next phase and Continue
  element(:closed_txt) { |b| b.span(css:".userplan .phase50 span:nth-child(1)")}

  element(:workshop_grades_link) { |b| b.a(css:"#workshop-viewlet-cleargrades_caption > a")}
  element(:clear_all_aggregated_grades1_btn) { |b| b.button(css:"input[value='clearaggregatedgrades'] ~ button")}
  element(:clear_all_aggregated_grades2_btn) { |b| b.input(css:"input[value='Clear all aggregated grades']")}
  element(:yes_btn) { |b| b.input(css:"input[value='Yes']")}
end
